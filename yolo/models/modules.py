...
class FPN(nn.Module):
    def __init__(self, channel, act_type, p2=False):
        super().__init__()
    ...
    def forward(self, x1, x2, x3, x4=None):
        ...
        if x4 is not None:
            # 1/4
            size4 = x4.size()[2:]
            x = F.interpolate(x2, size4, mode='nearest')
...
class PAN(nn.Module):
    def __init__(self, channel, act_type, p2=False):
        super().__init__()
        if p2:
            self.pan = PAN_4(channel, act_type, p2)
        else:
            self.pan = PAN_3(channel, act_type)

    def forward(self, x1, x2, x3, x4=None):
        if x4 is not None:
            return self.pan(x1, x2, x3, x4)
        else:
            return None, *self.pan(x1, x2, x3)

class PAN_3(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        # x1.shape
        # torch.Size([6, 384, 20, 20])
        # x2.shape
        # torch.Size([6, 192, 40, 40])
        # x3.shape
        # torch.Size([6, 96, 80, 80])
        
        # 1/32
        x1 = self.up1(x1)   # 192,20,20

        # 1/16
        size2 = x2.size()[2:]
        x = F.interpolate(x1, size2, mode='nearest') # 192,40,40
        x = torch.cat([x, x2], dim=1) # 384,40,40
        x2 = self.up2(x) # 96,40,40

        # 1/8
        size3 = x3.size()[2:]
        x = F.interpolate(x2, size3, mode='nearest') # 96,80,80
        x = torch.cat([x, x3], dim=1) # 192,80,80
        x_head1 = self.head1(x) # 96,80,80

        # 1/16
        x = self.down1(x_head1) # 96,40,40
        x = torch.cat([x, x2], dim=1) # 192,40,40
        x_head2 = self.head2(x) # 192,40,40

        # 1/32
        x = self.down2(x_head2) # 192,20,20
        x = torch.cat([x, x1], dim=1) # 384,20,20
        x_head3 = self.head3(x) # 384,20,20

        return x_head1, x_head2, x_head3

class PAN_4(nn.Module):
    def __init__(self, channel, act_type, p2 = False):
        super().__init__()
        self.up0 = ConvBNAct(channel, channel//2, 1, act_type=act_type)
        self.up1 = nn.Sequential(
                        ConvBNAct(channel, channel//4, 1, act_type=act_type),
                        ConvBNAct(channel//4, channel//4, 3, act_type=act_type),
        )
        self.up2 = nn.Sequential(
                        ConvBNAct(channel//2, channel//8, 1, act_type=act_type),
                        ConvBNAct(channel//8, channel//8, 3, act_type=act_type),
        )
        self.down0 = ConvBNAct(channel//8, channel//8, 3, 2, act_type=act_type)
        self.down1 = ConvBNAct(channel//4, channel//4, 3, 2, act_type=act_type)
        self.down2 = ConvBNAct(channel//2, channel//2, 3, 2, act_type=act_type)
 
        self.head0 = nn.Sequential(
                        DSConvBNAct(channel//4, channel//8, 1, act_type=act_type),
                        ConvBNAct(channel//8, channel//8, 3, act_type=act_type),
                    )

        self.head1 = nn.Sequential(
                        ConvBNAct(channel//4, channel//4, 3, act_type=act_type),
                        ConvBNAct(channel//4, channel//4, 1, act_type=act_type),
                    )
    ...
    def forward_4(self, x1, x2, x3, x4):
        # x1.shape
        # torch.Size([6, 384, 20, 20])
        # x2.shape
        # torch.Size([6, 192, 40, 40])
        ...
        # torch.Size([6, 96, 80, 80])
        # x4.shape
        # torch.Size([6, 48, 160, 160])

        # 1/32
        x1 = self.up0(x1)   #192,20,20

        # 1/16
        size2 = x2.size()[2:]
        x = F.interpolate(x1, size2, mode='nearest')
        x = torch.cat([x, x2], dim=1)   #384,40,40
        x2 = self.up1(x)    #96,40,40
        
        # 1/8
        size3 = x3.size()[2:]
        x = F.interpolate(x2, size3, mode='nearest')
        x = torch.cat([x, x3], dim=1) #192,80,80
        x3 = self.up2(x)    #48,80,80

        # 1/4
        size4 = x4.size()[2:]
        x = F.interpolate(x3, size4, mode='nearest')
        x = torch.cat([x, x4], dim=1)  #96,160,160
        x_head0 = self.head0(x) #48,160,160

        # 1/8
        size3 = x3.size()[2:]
        x = self.down0(x_head0)
        x = torch.cat([x, x3], dim=1)
        x_head1 = self.head1(x) # 96,80,80
        ...
        return x_head0, x_head1, x_head2, x_head3

    def forward(self, x1, x2, x3, x4=None):
        if x4 is None:
            return self.forward_3(x1, x2, x3)
        else:
            return self.forward_4(x1, x2, x3, x4)
