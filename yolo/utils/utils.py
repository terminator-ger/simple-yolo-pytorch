...
def xyxy_to_xywh(xyxy):
    ...
    width = x2 - x1
    height = y2 - y1
    assert height.min() >= 0 and width.min() >= 0, "Invalid bbox with negative width or height"
    assert height.max() > 0 and width.max() > 0, "Invalid bbox with zero width or height"
    assert (width > 0).all() and (height > 0).all(), "Invalid bbox with non-positive width or height"
    x_center = (x1 + x2) / 2
    y_center = (y1 + y2) / 2
...
def xywh_to_xyxy(xywh):
    ...
    x1 = x_center - abs(width / 2)
    y1 = y_center - abs(height / 2)
    x2 = x_center + abs(width / 2)
    y2 = y_center + abs(height / 2)
