...
from .loss.yolo_loss import get_loss_fn, YOLOLoss
from .loss.yolodetection_loss import YOLODetectionLoss
from .loss.yolokeypoint_loss import YOLOKeypointLoss
...
class YOLOTrainer(BaseTrainer):
    def __init__(self, config):
        ...
        self.losses = get_loss_fn(config)
        self.config = config

    def log_dict(self, metrics, prefix="", on_step=False, on_epoch=False):
        assert (on_step or on_epoch), "log_dict requires either on_step or on_epoch"
        assert (on_step ^ on_epoch), "Cannot log on_step and on_epoch at the same time"

        if on_step:
            counter = self.train_itrs
        if on_epoch:
            counter = self.cur_epoch

        for k,v in metrics.items():
            try:
                self.writer.add_scalar(f"{prefix}/{k}", v, counter)
    ...
    def train_one_epoch(self, config):
        ...
        for cur_itrs, data in enumerate(pbar):
            ...
            if config.use_tb and self.main_rank:
                self.log_dict(loss_dict, 'train', on_step=True)
            self.writer.add_scalar('train/loss', loss, self.train_itrs)
            ...
            if self.main_rank:
                pbar_description = f'Epoch {self.cur_epoch}/{config.total_epoch}{" "*4}|'
                pbar_description += f'Loss:{loss.item():4.4g}{" "*4}|'
                for k,v in loss_dict.items():
                    pbar_description += f'{k}:{v:4.2g}{" "*2}|'
    ...
    def validate(self, config, val_best=False):
            ...
            for k, prediction in batch_predictions.items():
                for batch_idx, p in enumerate(prediction):
                    ...
                    if k == ModelOutputKeys.Keypoints: 
                        pred_conf = p[:,0].to(self.config.val_device)
                        pred_classes = torch.ones((p.shape[0], 1), dtype=torch.float32, device=self.config.val_device)
                        pred_boxes = p[:, 1:5].to(self.config.val_device)
                        target_boxes   = data['keypoints_bboxes'][data['keypoints_bboxes'][:,0]==batch_idx][:,1:].to(self.config.val_device)
                        target_classes = data['keypoints_classes'][data['keypoints_classes'][:,0]==batch_idx][:,1:].to(self.config.val_device)
                        num_class = 1
                    else:
                        pred_conf = p[:,0].to(self.config.val_device)
                        pred_classes = p[:,5:].to(self.config.val_device)
                        pred_boxes = p[:,1:5].to(self.config.val_device)
                        target_boxes   = data['bboxes'][data['bboxes'][:,0]==batch_idx][:,1:].to(self.config.val_device)
                        target_classes = data['classes'][data['classes'][:,0]==batch_idx][:,1:].to(self.config.val_device)
                    ...
                    if k == ModelOutputKeys.Keypoints:
                        ...
                        pred[ModelOutputKeys.Keypoints] = {
                            E.EvalTypes.BBox: [{
                                "boxes": xywh_to_xyxy(p[select_idx][max_idx, 1:5]).reshape(-1,4),
                                "scores": p[select_idx][max_idx, 0].reshape(-1),
        ...
        if self.main_rank and config.use_tb and self.cur_epoch < config.total_epoch:
            ...
            self.log_dict(metrics, 'val', on_epoch=True)
            ...
            map50 = torch.tensor(0)
            if 'B_map_Stones' in metrics:
    ...
    def predict(self, config):
        ...
        for (images, images_aug, img_names) in tqdm(self.test_loader):
            ...
            for i, image_torch in enumerate(images):
                image = Image.fromarray(image_torch.detach().cpu().numpy())
                save_path = os.path.join(config.save_dir,'imgs', img_names[i])

                if str(ModelOutputKeys.Detections) in preds:
                    pred = preds[str(ModelOutputKeys.Detections)][i]
                    pred_conf = pred[:, 0]
                ...
                if str(ModelOutputKeys.Keypoints) in preds:
                    kp = preds[str(ModelOutputKeys.Keypoints)][i]
                    kp_conf = kp[:, 0]
                    kp_boxes = xywh_to_xyxy(kp[:, 1:5])
                    kp_boxes[:, 0::2].clamp_(0, width)
                    kp_boxes[:, 1::2].clamp_(0, height)
                    kps = kp[:,5:]
                    #cls_logits, pred_cls = pred[:, 5:].max(dim=1)
                    #pred_conf *= cls_logits
                    kp = torch.cat([kp_conf.unsqueeze(1), kp_boxes, kps], dim=1)
                    output = kp[kp_conf > config.test_conf_thrs]

                    # NMS per image
                    kept_indices = self.nms(output, nms_iou=config.test_iou)

                    if kept_indices is not None:
                        kps_offset = 5 + self.config.num_class_kp
                        output = output[kept_indices].cpu()
                        conf = output[:, 0]
                        max_conf = torch.argmax(conf)
                        conf = conf[max_conf].unsqueeze(0)
                        bbox = output[max_conf, 1:5].unsqueeze(0)
                        kps =  output[max_conf, kps_offset:].unsqueeze(0)
                        xy = [(x,y) for x,y in kps.reshape(-1,2)]
                        ...
                        for j in range(conf.shape[0]):
                            #text = f'{config.class_map[cls[j].item()]}-{conf[j]:.2f}'
                            draw.rectangle(bbox[j].tolist(), outline=tuple(config.color_map[0]), width=2)
                            draw.polygon(xy)
                            #draw.text(bbox[j,:2].tolist(), text, fill='white', font=font)
    ...
    def debug(self, config):
        ...
        for cur_itrs, data in enumerate(tqdm(self.train_loader)):
            bboxes = data['bboxes'].float()
            classes = data['classes'].float()
            kp_bboxes = data['keypoints_bboxes'].float()
            kp_classes = data['keypoints_classes'].float()

            if cur_itrs >= config.num_debug_batch:
                break

            #assigned_labels = YOLODetectionLoss(config).label_assignment(bboxes, classes, config.train_bs, len(config.anchor_boxes))
            ...
            assigned_labels = YOLOKeypointLoss(config).label_assignment(kp_bboxes, kp_classes, config.train_bs, len(config.anchor_boxes))

            visualize_assignments(assigned_labels=assigned_labels, bboxes=kp_bboxes, anchor_boxes=config.anchor_boxes,
                                    batch_idx=cur_itrs, img_size=config.img_size, image_stride=(4, 8, 16, 32), save_dir=config.debug_dir)
