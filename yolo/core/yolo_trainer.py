        ...
        for (images, bboxes, classes) in pbar:
            ...
            for i, pred in enumerate(preds):
                ...
                if False:
                    ...
                    save_path = os.path.join(config.save_dir, f"{i}_pred.jpg")

                    t_bboxes = targets[-1]['boxes']
                    t_classes = targets[-1]['labels']
                    for bbox, cls in zip(t_bboxes, t_classes):
                        bbox = bbox.long()
                        draw = ImageDraw.Draw(image)

                        text = f'{config.class_map[cls.item()]}-gt'
                        draw.rectangle(bbox.tolist(), outline=(255,0,0), width=2)
                        draw.text(bbox[:2].tolist(), text, fill='white', font=font)
